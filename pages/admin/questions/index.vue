...
export default {
  ...
  methods: {
    fetchQuestions() {
      this.$axios.get(`https://nuxt-quiz-70f35-default-rtdb.firebaseio.com/questions.json`).then(
        res =>
    ...
    destroy(key) {
      this.$axios.delete(`https://nuxt-quiz-70f35-default-rtdb.firebaseio.com/questions/${key}.json`).then(res => {
    ...
    fetchAnswer(key) {
      this.$axios
        .get(
          `https://nuxt-quiz-70f35-default-rtdb.firebaseio.com/answers.json?orderBy="question_id"&startAt="${key}"&endAt="${key}"`
        )
        .then(res => this.destroyAnswer(key, Object.keys(res.data)[0]))
    },
    destroyAnswer(key, answerId) {
      this.$axios
        .delete(`https://nuxt-quiz-70f35-default-rtdb.firebaseio.com/answers/${answerId}.json`)
